...
class Home extends Controller
{
	...
	{	
		...
		if(session()->get('role_id') == 1 || session()->get('role_id') == 2){
			$data["sanstha_state"] = (new SansthaModel())->distinct()->select('cs_state')->where(array('cs_isDelete'=> 0))->findAll();
		}else{
			$data["sanstha_state"] = (new statesModel())->select('st_id as cs_state')->where(array('st_id'=>session()->get('state')))->findAll();			
		}
